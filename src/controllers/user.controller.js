...
const refreshAccessToken = asyncHandler(async (req, res) => {
  const incomingRefshtoken = req.cookies?.refreshToken || req.body.refreshToken;

  if (!incomingRefshtoken) {
  ...
  try {
    ...
    if (incomingRefshtoken !== user?.refreshToken) {
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  // select method remove the fields from return object
...
const updateUsercoverImage = asyncHandler(async (req, res) => {
  ...
  const coverImage = await uploadFileOnCloudinayry(coverLocalPath);
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  if (!channel?.length) {
    return ApiError(404, "channel does not exist");
  }
